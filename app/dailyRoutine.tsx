        ...
        {isLoading || isFetching ? (
          ...
          <>
            <View className="w-full h-full flex justify-center items-center my-4 px-4 mt-28">
              <BackButton />
              <Text className="text-3xl font-bold text-center">Daily Routine Detail</Text>

              <Text className="text-2xl font-medium text-left px-4 mt-4 mb-8">{dayName}</Text>

              <View className="w-full flex-row text-left">
                <RefreshButton onRefresh={() => refreshExercise(0)} />
                <Text className="pb-0 pt-1">Swap an Exercise</Text>
              </View>
              ...
              {exerciseDetails.map((item) => (
                <View key={item.exerciseDetailId + 1} className="w-full py-1">
                  ...
                  <View className="flex-row items-center">
                    ...
                    >
                      <Text className="flex-[3] text-center">{item.exerciseName}</Text>
                      <Text className="flex-[1] text-center">{item.sets}</Text>
                ...
                <View key={item.exerciseDetailId} className="w-full flex-row">
                  <View className="relative mr-2">
                    ...
                    <TouchableOpacity
                      onPress={() => {
                        setCurrentVideoId(
                          item.youtubeURL.substring(item.youtubeURL.lastIndexOf("=") + 1),
                        );
                  ...
                  >
                    ...
                    <View className="flex-row flex-[3] items-center">
                      <Text className="flex-[2] text-center">{item.exerciseName}</Text>
                      <Text className="flex-[1] text-center">{item.sets}</Text>
                      <Text className="flex-[1] text-center">
                        {item.reps ? item.reps : item.minutes}
                      </Text>
                    ...
                    <TouchableOpacity
                      className="flex-[1]"
                      onPress={() => displayModal(item.exerciseDetailId)}
                    >
              ...
              <Modal isVisible={showModal} onBackdropPress={() => setShowModal(false)}>
                <View style={{ backgroundColor: "white", padding: 20, borderRadius: 10 }}>
                  ...
                  <Text className="text-lg font-pregular text-black">
                    {modalContent?.reps !== undefined && modalContent.reps > 0
                      ? `Reps: ${modalContent.reps}`
                      : `Mins: ${modalContent?.minutes ?? 0}`}
                  </Text>
                  <Text className="text-lg font-pregular text-black">
                  ...
                  <Text className="text-lg font-pregular text-black">
                    Environment:{" "}
                    {modalContent?.workoutEnvs.map((env) => env.description).join(", ")}
                  </Text>
                  <Text className="text-lg font-pregular text-black">
                    Muscle Group:{" "}
                    {modalContent?.muscleGroups.map((muscle) => muscle.description).join(", ")}
                  </Text>
              ...
              >
                <View
                  style={{ backgroundColor: "black", padding: 0, borderRadius: 10, width: "100%" }}
                >
                  {/* Close Button */}
                  <TouchableOpacity
                  ...
                  >
                    <Text
                      style={{ fontSize: 24, fontWeight: "bold", color: "white", marginBottom: 2 }}
                    >
                      X
                    </Text>
                  </TouchableOpacity>
