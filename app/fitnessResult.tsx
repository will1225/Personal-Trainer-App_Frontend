...
const FitnessResult = () => {
  ...
  useEffect(() => {
    const getFitnessData = async () => {
      ...
      if (fitnessResult.ranges.classifications && fitnessResult.ffmiTable) {
        const fatLevel = fitnessResult.ranges.classifications.map(
      ...
      if (
        fitnessResult.classification === "Out of classification range" ||
        fitnessResult.ffmiClassification === "Unusual/Extreme Result"
      ) {
        Alert.alert(
          "WARNING",
      ...
      const updateResult = await fitnessUtil.saveIntensityAndLevel(
        fitnessResult.classification,
        fitnessResult.ffmiClassification,
      );
      ...
      if (isProgress) {
        ...
        const prevFitnessResult = await fitnessUtil.fetchFitnessResult(
          progressResult.prevMeasurementId,
        );
  ...
  const isHighlighted = (range: { classification: string; min: number; max: number }) => {
    ...
    return (
      currentFitnessResult.bodyFatPercent >= range.min &&
      (range.max === null || currentFitnessResult.bodyFatPercent <= range.max)
    );
  };
    ...
    <SafeAreaView className="flex-1">
      <ScrollView contentContainerStyle={{ flexGrow: 2, justifyContent: "center" }}>
        <View className="w-full h-full flex justify-center items-center my-4 px-4 mt-28">
          <BackButton />
          <Text className="text-3xl font-bold text-center mb-8">Your Current Fitness Level</Text>
          ...
          {loading ? (
            ...
            <>
              ...
              <View className={`flex flex-col items-center mb-6 mt-8 w-full px-2`}>
                <View
                  className={`flex flex-row ${!isProgress ? "justify-center space-x-8" : "justify-between"} items-center mb-3 w-full`}
                >
                  <Text className="text-xl font-semibold text-right w-[28%]">{``}</Text>
                  {isProgress && (
                    <>
                      <Text className="text-lg w-[28%] text-center truncate font-semibold">
                        {new Date(prevFitnessResult.date).toLocaleDateString(undefined, options)}
                      </Text>
                      <Text className="text-xl w-[14%] text-center">{``}</Text>
                    </>
                  )}
                  <Text className="text-xl w-[28%] text-center truncate font-semibold">
                    {new Date(currentFitnessResult.date).toLocaleDateString(undefined, options)}
                  </Text>
                </View>
                ...
                >
                  ...
                  {isProgress && (
                    <>
                      <Text className="text-xl w-[28%] text-center truncate">
                        {prevFitnessResult.bodyFatPercent}
                      </Text>
                      <Text
                        className={`text-xl w-[14%] text-center ${
                          currentFitnessResult.bodyFatPercent - prevFitnessResult.bodyFatPercent ===
                          0
                            ? {}
                            : currentFitnessResult.bodyFatPercent -
                                  prevFitnessResult.bodyFatPercent >
                                0
                              ? "text-red-500"
                              : "text-green-500"
                ...
                >
                  ...
                  {isProgress && (
                    <>
                      <Text className="text-xl w-[28%] text-center truncate">
                        {prevFitnessResult.muscleMass}
                      </Text>
                      <Text
                        className={`text-xl w-[14%] text-center ${
                          prevFitnessResult.muscleMass - currentFitnessResult.muscleMass === 0
                            ? {}
                            : prevFitnessResult.muscleMass - currentFitnessResult.muscleMass > 0
                              ? "text-red-500"
                              : "text-green-500"
                  ...
                  <Text className="text-xl w-[28%] text-center truncate">
                    {currentFitnessResult.muscleMass} kg
                  </Text>
                </View>
                ...
                >
                  ...
                  {isProgress && (
                    <>
                      <Text className="text-xl w-[28%] text-center truncate">
                        {prevFitnessResult.weight}
                      </Text>
                      <Text className={`text-xl w-[14%] text-center`}>{`\u2794`}</Text>
                    </>
                  )}
                  <Text className="text-xl w-[28%] text-center truncate">
                    {currentFitnessResult.weight} kg
                  </Text>
                </View>
                ...
                >
                  ...
                  {isProgress && (
                    <>
                      <Text className="text-lg w-[28%] text-center truncate">
                        {prevFitnessResult.classification}
                      </Text>
                      <Text
                        className={`text-xl w-[14%] text-center
                      ${
                        fatLevel.indexOf(prevFitnessResult.classification) -
                          fatLevel.indexOf(currentFitnessResult.classification) ===
                        0
                          ? {}
                          : fatLevel.indexOf(prevFitnessResult.classification) -
                                fatLevel.indexOf(currentFitnessResult.classification) <
                  ...
                  <Text className="text-lg w-[28%] text-center truncate">
                    {currentFitnessResult.classification}
                  </Text>
                </View>
                ...
                >
                  ...
                  {isProgress && (
                    <>
                      <Text className="text-lg w-[28%] text-center truncate">
                        {prevFitnessResult.ffmiClassification}
                      </Text>
                      <Text
                        className={`text-xl w-[14%] text-center
                      ${
                        ffmiLevel.indexOf(prevFitnessResult.ffmiClassification) -
                          ffmiLevel.indexOf(currentFitnessResult.ffmiClassification) ===
                        0
                          ? {}
                          : ffmiLevel.indexOf(prevFitnessResult.ffmiClassification) -
                                ffmiLevel.indexOf(currentFitnessResult.ffmiClassification) >
                  ...
                  <Text className="text-lg w-[28%] text-center truncate">
                    {currentFitnessResult.ffmiClassification}
                  </Text>
                </View>

                {currentFitnessResult.age < 18 ? (
                  <Text className="text-base font-medium text-left mt-4">
                    Note: If you are under 18 yrs old, Fat Level is calculated based on the 18 - 29
                    age group.
                  </Text>
                ) : (
                  ""
              ...
              <View className="flex flex-col items-center px-1 mb-6 mt-2">
                <Text className="text-sm font-medium text-left">
                  ** Fat Level reflects your body fat for your age, while FFMI indicates your muscle
                  mass relative to your height.
                </Text>
              </View>

              {/* Body Fat Percentage Table */}
              <BodyFatChart
                ranges={currentFitnessResult.ranges.classifications}
                gender={gender}
                isHighlighted={isHighlighted}
              />

              <Text className="text-xl text-left mb-8">Ready to get in shape?</Text>
