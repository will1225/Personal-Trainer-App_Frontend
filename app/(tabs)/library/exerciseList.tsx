...
const ExerciseList = () => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const workoutEnvs = [
          deselectOption,
          ...new Map(
            exercisesWithFetchedDetails.flatMap((ex) => ex.env).map((env) => [env.id, env]),
          ).values(),
        ];
        const levels = [
          deselectOption,
          ...new Map(exercisesWithFetchedDetails.map((ex) => [ex.levelId, ex.level])).values(),
        ];
        const equipment = [
          deselectOption,
          ...new Map(
            exercisesWithFetchedDetails.map((ex) => [ex.requiredEquipmentId, ex.equipment]),
          ).values(),
        ];
        const types = [
          deselectOption,
          ...new Map(exercisesWithFetchedDetails.map((ex) => [ex.typeId, ex.type])).values(),
        ];
  ...
  const filterExercises = useCallback(() => {
    return exercises.filter((exercise) => {
      const matchesEnv = selectedEnv
        ? exercise.env.some((env: { id: string }) => env.id === selectedEnv)
        : true;
      const matchesLevel = selectedLevel ? exercise.level.id === selectedLevel : true;
      const matchesEquipment = selectedEquipment
        ? exercise.equipment.id === selectedEquipment
        : true;
  ...
  useEffect(() => {
    ...
    const activatedFilters =
      !!selectedEnv || !!selectedLevel || !!selectedEquipment || !!selectedType; // explicit way to convert to boolean
    ...
    <SafeAreaView>
      <ScrollView contentContainerStyle={{ flexGrow: 2, justifyContent: "center" }}>
        <View className="w-full h-full flex justify-center items-center my-4 px-4 mt-28">
          ...
          {/* Muscle Group header */}
          <Text className="text-3xl font-bold text-center mb-12">{description}</Text>
          ...
          <View className="flex-row justify-end w-full mt-2 mr-4">
            ...
            >
              <Text className="font-psemibold text-base">Reset Filters</Text>
            </TouchableOpacity>
          </View>
          ...
          {!isLoading && exercises && filteredExercises.length > 0 && (
            <View className="w-full flex-row text-left mt-4">
              ...
              <Text className="ml-1 pb-2 pt-1">Refresh a new video</Text>
            </View>
          )}
          ...
          {!isLoading &&
            ...
            (filtersActivated && filteredExercises.length === 0 ? (
              <Text className="font-psemibold text-base mt-20">
                ...
                <View key={index} className="flex-row mb-1 items-center">
                  <View className="relative">
                    ...
                    <TouchableOpacity
                      onPress={() => {
                        setCurrentVideoId(
                          exercise.videoUrl.substring(exercise.videoUrl.lastIndexOf("=") + 1),
                        );
                        setModalContent(exercise);
                  ...
                  >
                    ...
                    <View className="flex-row items-center">
                      <Text className="flex-[2] text-center">{exercise.name}</Text>
                      <Text className="flex-[1] text-center">{exercise.defaultSets}</Text>
          ...
          <Modal isVisible={showModal} onBackdropPress={() => setShowModal(false)}>
            <View style={{ backgroundColor: "white", padding: 20, borderRadius: 10 }}>
              ...
              <Text className="text-lg font-pregular" style={{ color: "#333" }}>
                ...
                {"\n"}
                {modalContent.defaultReps
                  ? `Default Reps: ${modalContent.defaultReps}${"\n"}`
        ...
        >
          <View style={{ backgroundColor: "black", padding: 0, borderRadius: 10, width: "100%" }}>
            ...
            >
              <Text style={{ fontSize: 24, fontWeight: "bold", color: "white", marginBottom: 2 }}>
                X
              </Text>
            </TouchableOpacity>
            ...
            <YoutubePlayer key={currentVideoId} height={250} play={true} videoId={currentVideoId} />
          </View>
        </Modal>
      </ScrollView>
    </SafeAreaView>
